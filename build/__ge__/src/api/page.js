...
export function create(payload) {
  assert(payload.componentName, 'api/page/create: payload should have componentName');
  ...
  assert(!existsSync(filePath), 'api/page/create: file exists');
...
export function update(payload) {
  assert(payload.source, 'api/page/update: payload should have source');
  const filePath = join(payload.sourcePath, payload.filePath);
...
export function addDispatch(payload) {
  assert(payload.actionType, 'api/page/addDispatch: payload should have actionType');
  const filePath = join(payload.sourcePath, payload.filePath);
  const source = readFile(filePath);
  const root = j(source);

  root.findpage().forEach(path => {
