...
export default {
  ...
  state: {
    ...
    defaultLocale: 'en',
    antd: null,
    localeList: ['zh-CN', 'zh-HK', 'en-US','vi-VN','th-TH','ko-KR','ms-MY','ja-JP','km-KM','id-ID', 'mm-MM','en-PH','es-ES','de-DE','it-IT','pt-PT','da-DK','se-SE','nb-NO','bg-BG','el-GR','pl-PL','ro-RO','cs-CZ','hu-HU','sk-SK','pl-PL', ],
    languageList: [
  ...
  subscriptions: {
    setup({ history, dispatch }: SubscriptionAPI) {
      dispatch({type: 'list'});
  ...
  effects: {
    ...
    *load({ payload }: DvaAction, {call, put, take}: EffectsCommandMap) {
      ...
      if (Array.isArray(messages)) {
        ...
        yield put({type: "changeSuccess", payload: {
          loading: false,
          locale: locale,
          site: site
        }});
